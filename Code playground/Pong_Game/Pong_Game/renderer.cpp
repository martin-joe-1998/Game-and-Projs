...
internal void draw_text(const char* text, float x, float y, float size, u32 color) {
	...
	while (*text) {
		if (*text != 32) {
			...
			else if (*text == 48) letter = letters[26]; // '.'
...
// draw scores of both player at the top of the window
internal void draw_number(int number, float x, float y, float size, u32 color) {
	float half_size = size * .5f;
	bool drew_number = false; // make 0's drawing works at the beginning
